...
#define OS_TASK_START(task_id) (BIT_SET(os_ready_mask, (task_id - 1U)))
#define OS_TASK_STOP(task_id) (BIT_CLEAR(os_ready_mask, (task_id - 1U)))

#define OS_TASK_DELAY_SET(task_id) (BIT_SET(os_delay_mask, (task_id - 1U)))
#define OS_TASK_DELAY_CLEAR(task_id) (BIT_CLEAR(os_delay_mask, (task_id - 1U)))
...
#define OS_MUTEX(...)
#define OS_TASK(name, cb, stack, autostart, argv) \
    static os_stack_t name##_stack[stack];
#define OS_ALARM_TASK(name, stack) \
    static os_stack_t name##_stack[stack];

/* This macro creates the stack arrays for all tasks */
#define OS_CREATE_STACK_VAR() OS_PRIORITY_LIST()

static os_stack_t os_idle_task_stack[OS_IDLE_TASK_STACK_SIZE];
OS_CREATE_STACK_VAR();
...
#define OS_CREATE_STACK_ARRAY() OS_PRIORITY_LIST()
...
#define OS_ALARM_TASK(name, ...) \
    static os_task_t name##_control;

/* This macro creates the task control structure for all tasks */
#define OS_CREATE_TASK_CONTROL_BLOCK() OS_PRIORITY_LIST()
...
#if (OS_USE_FAST_STACK_MONITOR == true)
...
#elif (OS_USE_USER_STACK_MONITOR == true)
static inline void os_task_stack_mon(void)
{
    const os_task_t *const task = os_task_current;
    const os_stack_t pattern = OS_TASK_STACK_PATTERN;
...
#endif /* OS_USE_USER_STACK_MONITOR */
#endif /* OS_USE_TASK_STACK_MONITOR */
...
{
    OS_ASSERT(os_tasks[priority] == NULL, OS_MODULE_ID_TASK, OS_ERROR_INVALID_PARAM);
    OS_ASSERT(task != NULL, OS_MODULE_ID_TASK, OS_ERROR_NULLPTR);
    OS_ASSERT(task_handler != NULL, OS_MODULE_ID_TASK, OS_ERROR_NULLPTR);
    OS_ASSERT(stack != NULL, OS_MODULE_ID_TASK, OS_ERROR_NULLPTR);
    OS_ASSERT(priority < OS_TASK_MAX, OS_MODULE_ID_TASK, OS_ERROR_INVALID_PARAM);
...
{
    /* Start from the max priority, since OS_PRIORITY_LIST
    ...
    #define OS_TASK(name, cb, stack, autostart, argv)         \
        ...
        OS_TRACE_TASK_CREATE(&name##_control, #name, stack);  \
        priority--;
    ...
    OS_TRACE_TASK_CREATE(&os_idle_task_control, "OS_TASK_IDLE",
...
static void os_tasks_start(void)
{
    ...
    #define OS_TASK_START_ALL() OS_PRIORITY_LIST()
    OS_TASK_START_ALL();
}
...
{
    OS_ASSERT(id > OS_TASK_IDLE, OS_MODULE_ID_TASK, OS_ERROR_INVALID_PARAM);
    OS_ASSERT(id < OS_TASK_MAX, OS_MODULE_ID_TASK, OS_ERROR_INVALID_PARAM);
    OS_ASSERT(id < 255U, OS_MODULE_ID_TASK, OS_ERROR_INVALID_PARAM);

    const uint8_t priority = OS_TASK_MAX - id;

    os_enter_critical_section();

    OS_TASK_START(priority);
    OS_TASK_DELAY_CLEAR(priority);
    OS_TRACE_TASK_READY(os_tasks[priority]);
...
void os_delay(const uint32_t ticks)
{
    ...
    OS_TASK_DELAY_SET(os_task_current->priority);
    OS_TASK_STOP(os_task_current->priority);
    OS_TRACE_TASK_DELAYED(os_task_current);
...
void os_task_tick(void)
{
    ...
    while (mask)
    {
        ...
        if (task->ticks == 0)
        {
            ...
            OS_TASK_START(task->priority);
            OS_TASK_DELAY_CLEAR(task->priority);
