/******************************************************************************************
 * @brief Header File for BeeRTOS Assertion and Error Handling
 * @file BeeRTOS_assert.h
 * This header file defines the assertion mechanism and error handling interface for BeeRTOS.
 * It includes macros for performing assertions within the OS and user-defined modules, structures
 * for logging errors, and enumerations for module and error identifiers. The file facilitates
 * debugging and system integrity checks by allowing for conditional error reporting based on
 * expression evaluation and supports optional logging of error history for analysis.
 ******************************************************************************************/

#ifndef __BEERTOS_ASSERT_H__
#define __BEERTOS_ASSERT_H__

/******************************************************************************************
 *                                        INCLUDES                                        *
 ******************************************************************************************/

#include "BeeRTOS_internal.h"
#include "BeeRTOS_assert_cfg.h"

/******************************************************************************************
 *                                         DEFINES                                        *
 ******************************************************************************************/

/*! Assert macro - calls os_report_error() if the expression is false */
#define OS_ASSERT(expr, module_id, error_id)      \
    if (!(expr))                                  \
    {                                             \
        os_report_error((module_id), (error_id)); \
    }

/******************************************************************************************
 *                                        TYPEDEFS                                        *
 ******************************************************************************************/

#if (OS_USE_ASSERT_HISTORY_LOG == true)
/*! Error log structure used to store the last OS_ASSERT_HISTORY_LOG_SIZE errors */
typedef struct
{
    uint8_t module_id;
    uint8_t error_id;
    uint32_t timestamp;
} os_error_t;
#endif

#undef OS_ASSERT_USER
#define OS_ASSERT_USER(module_id, ...)  module_id,
/*! List of module IDs used in OS, also contains user defined module IDs, generated by
 *  OS_ASSERT_USER macro in BeeRTOS_assert_cfg.h */
typedef enum
{
    OS_MODULE_ID_KERNEL,
    OS_MODULE_ID_TASK,
    OS_MODULE_ID_ALARM,
    OS_MODULE_ID_MUTEX,
    OS_MODULE_ID_SEMAPHORE,
    OS_MODULE_ID_QUEUE,
    OS_MODULE_ID_MESSAGE,

    OS_ASSERT_USER_LIST()

    OS_MODULE_ID_MAX
} os_assert_module_id_t;

#undef OS_ASSERT_USER
#define OS_ASSERT_USER(module_id, ...)  __VA_ARGS__,
/* List of error IDs used in OS, also contains user defined error IDs, generated by
 * OS_ASSERT_USER macro in BeeRTOS_assert_cfg.h */
typedef enum
{
    OS_ERROR_NOT_INITIALIZED,
    OS_ERROR_INVALID_PARAM,
    OS_ERROR_INVALID_STATE,
    OS_ERROR_INVALID_OPERATION,
    OS_ERROR_NO_MEMORY,
    OS_ERROR_NO_RESOURCE,
    OS_ERROR_NO_SPACE,
    OS_ERROR_NO_DATA,
    OS_ERROR_TIMEOUT,
    OS_ERROR_OVERFLOW,
    OS_ERROR_NULLPTR,

    OS_ASSERT_USER_LIST()

    OS_ERROR_ID_MAX
} os_assert_error_id_t;

/******************************************************************************************
 *                                    GLOBAL VARIABLES                                    *
 ******************************************************************************************/

/******************************************************************************************
 *                                   FUNCTION PROTOTYPES                                  *
 ******************************************************************************************/
void os_report_error(const uint8_t module_id, const uint8_t error_id);
os_error_t* os_assert_get_history_log(void);

#endif /* __BEERTOS_ASSERT_H__ */