 ...
 *
 *  Structure for defining a task: OS_TASK(task_id, function, stacksize, autostart, task_arg)
 ...
 *
 *  Structure for defining a mutex: OS_MUTEX(mutex_id, initial_count)
 ...
 *  handling all alarms.
 *
 *  Structure for defining the alarm task: OS_ALARM_TASK(task_id, stacksize)
 ...
 */
#define OS_PRIORITY_LIST()                                               \
    OS_TASK(OS_TASK_UT_MAIN, ut_beertos_main_task, 128, true, NULL)      \
    OS_ALARM_TASK(OS_ALARM_TASK, 128)                                    \
    /* Priority test tasks */                                            \
    OS_TASK(OS_TASK_PRIO_3, ut_task_priority_highest, 128, true, NULL)   \
    OS_TASK(OS_TASK_PRIO_2, ut_task_priority_medium, 128, true, NULL)    \
    OS_TASK(OS_TASK_PRIO_1, ut_task_priority_lowest, 128, true, NULL)    \
    /* Mutex test tasks */                                               \
    OS_MUTEX(MUTEX_ONE, 0U)                                              \
    OS_MUTEX(MUTEX_THREE, 0U)                                            \
    OS_TASK(OS_TASK_MUTEX_3, mutex_high_priority_task, 128, false, NULL) \
    OS_TASK(OS_TASK_MUTEX_1, mutex_low_priority_task, 128, false, NULL)  \
    /* Message test tasks */                                             \
    OS_TASK(OS_TASK_MSG_2, ut_task_msg_2, 128, false, NULL)              \
    OS_TASK(OS_TASK_MSG_1, ut_task_msg_1, 128, false, NULL)
 ...
 *
 * Structure: OS_SEMAPHORE(semaphore_id, initial_count, type)
 ...
 */
#define OS_SEMAPHORE_LIST()                                   \
    OS_SEMAPHORE(SEMAPHORE_UT1, 10U, SEMAPHORE_TYPE_COUNTING) \
    OS_SEMAPHORE(SEMAPHORE_UT2, 0U, SEMAPHORE_TYPE_BINARY)    \
    OS_SEMAPHORE(SEMAPHORE_TWO, 0U, SEMAPHORE_TYPE_BINARY)

/*!
 *  @brief Define your alarms here
 *  @note Structure: OS_ALARM(alarm_id, callback, autostart, default_period, periodic)
 *
 ...
 */
#define OS_ALARM_LIST()                                    \
    OS_ALARM(ALARM_ONE, alarm1_callback, false, 0U, false) \
    OS_ALARM(ALARM_TWO, alarm2_callback, false, 0U, false) \
    OS_ALARM(ALARM_THREE, alarm3_callback, false, 0U, false)
...
extern void mutex_low_priority_task(void *arg);
extern void mutex_high_priority_task(void *arg);
