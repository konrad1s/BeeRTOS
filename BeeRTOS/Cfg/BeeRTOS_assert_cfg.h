 ...
 * The size of the log is defined by OS_ASSERT_HISTORY_LOG_SIZE.
 * The log can be read using os_assert_get_history_log() function.
 */
#define OS_USE_ASSERT_HISTORY_LOG   (true)
#define OS_ASSERT_HISTORY_LOG_SIZE  (10U)

/*! @brief User defined error handler
 * Error handler called when an assertion is triggered.
 * 
 * Structure OS_ASSERT_ERROR_HANDLER(module_id, error_id)
 * @param module_id - module id of the assertion
 * @param error_id - error id of the assertion
 */
#define OS_ASSERT_ERROR_HANDLER(module_id, error_id) \
    while (1);
 ...
 * The error handler can be defined in the application using OS_ASSERT_ERROR_HANDLER(module_id, error_id) macro.
 * 
 * Structure OS_ASSERT_USER:
 * OS_ASSERT_USER(module_id, error_id_1, ...)
 ...
 */
#define OS_ASSERT_USER_LIST()                                                          \
    /* module id, list of error ids */                                                 \
    OS_ASSERT_USER(OS_ASSERT_USER1, OS_USER_ERROR_1, OS_USER_ERROR_2, OS_USER_ERROR_3) \
    OS_ASSERT_USER(OS_ASSERT_USER2, OS_USER_ERROR_10, OS_USER_ERROR_11, OS_USER_ERROR_12, OS_USER_ERROR_13)
